...
public class TagFilter {
    ...
    public enum styles{italic, bold, underlined, superscript, none,remove,paragraph};
    public enum noteStyles{footnote, endnote, sidebyside, inline, remove};
            ...
            for(int charCounter=0;charCounter<this.text.length();charCounter++)
            {
                ...
                if(text.charAt(charCounter)=='>')
                {
                    ...
                    else
                    {
                        ...
                        {
                            if(chunkStyle!=null)
                            System.out.print(" closing tag "+tagTextBuffer+" with style "+chunkStyle.name()+"\n");
                            else
