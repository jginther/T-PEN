...
DatabaseWrapper.closePreparedStatement(ps);
    ...
    {
      String query="insert into manuscript (repository, archive, msIdentifier,city) values(?,?,?,?)";
      Connection j=null;
PreparedStatement ps=null;
      try{
          j=DatabaseWrapper.getConnection();
...
DatabaseWrapper.closePreparedStatement(ps);
    ...
    public static Manuscript[] getManuscriptsByCityAndRepository(String city, String repo) throws SQLException
        ...
        String query = "select id from manuscript where repository=? and city=? and restricted!=-999 order by collection" ;
        Connection j = null;
PreparedStatement ps=null;
        try
            {
...
DatabaseWrapper.closePreparedStatement(ps);
    ...
    public static Manuscript[] getManuscriptsByRepository(String repo) throws SQLException
        {
        Manuscript[] toret = new Manuscript[0];
        String query = "select id from manuscript where repository=? and restricted!=-999 order by collection";
        Connection j = null;
PreparedStatement ps=null;
        try
            {
...
DatabaseWrapper.closePreparedStatement(ps);
    ...
    public static String getFullDocument(Project p, TagFilter.noteStyles includeNotes, Boolean newLine, Boolean pageLabels, Boolean imageWrap) throws SQLException
        ...
        String toret = "";
        String query = "select transcription.text, transcription.comment, transcription.folio from transcription join folios on folios.pageNumber=transcription.folio where projectID=? order by folio,x,y";
        Connection j = null;
PreparedStatement ps=null;
