 ...
 */
/**
 * A button that creates open and close tags
 */
package textdisplay;
...
/**
     ...
     * @return the tag description, or the tag text if the description hasnt been set
     */
...
DatabaseWrapper.closePreparedStatement(ps);
    ...
    public String getButton() {
        ...
        if(!caller)
        Logger.getLogger(TagButton.class.getName()) .log(Level.SEVERE,formatter.format(date)+" Running tagButton.getAllProjectButtons\n"+stackTrace );
        String toret = "";
        if (description.length() > 0) {
            toret = "<span class=\"lookLikeButtons\" title=\"" + getFullTag() + "\" onclick=\"Interaction.insertTag('" + this.tag + "', '" + getFullTag() + "');\">" + description  + "</span>";
        } else {
            toret = "<span class=\"lookLikeButtons\" title=\"" + getFullTag() + "\" onclick=\"Interaction.insertTag('" + this.tag + "', '" + getFullTag() + "');\">" + this.tag     + "</span>";
        }
        return toret;
    }
...
DatabaseWrapper.closePreparedStatement(stmt);
    ...
    public static String[][] getTagsFromSchema(Project p, Vector<parameterWithValueList> v) throws SQLException, MalformedURLException, IOException {
        try {
