...
public class Project {
    ...
    public void addUserTool(String name, String url) throws SQLException {
        new utils.UserTool(name, url, this.projectID);
    }

    /**Add the standard latin tools to a project*/
    public void initializeTools() throws SQLException {
    ...
    /**Set the priority number for this project, which is used to sort the project listing. Higher is displayed first*/
    ...
    /**Create a new Project with the specified leader and copes of the transcriptions, annotations, buttons, and schema*/
    ...
    /**Runs line detection on predefined columns. The columns are defined as lines when this is called, so you might have 2 large lines, each being a column.*/
    public void detectInColumns(int folioNumber) throws SQLException {
        Folio f = new Folio(folioNumber);
        Line[] t = this.getLines(folioNumber);
        Logger.getLogger("project").log(Level.SEVERE, ("User specifiecd column count " + t.length + "\n"));
        for (int i = 0; i < t.length; i++) {
            Logger.getLogger("project").log(Level.SEVERE, ("Column " + t[i].left + "," + t[i].right + "," + t[i].top + "," + t[i].bottom + "\n"));
        }

        Line[] linePositions = null;
        try {
            linePositions = f.detectInColumns(t);
        } catch (Exception e) {
            Logger.getLogger("project").log(Level.SEVERE, ("Parsing error\n" + e.getLocalizedMessage()));
        }

        Connection j = null;
        PreparedStatement stmt = null;
        try {
            Logger.getLogger("folio").log(Level.SEVERE, ("Recieved " + linePositions.length + " lines in PROJECT on line 298"));
            j = DatabaseWrapper.getConnection();
    ...
    /**Update the linebreak text by removing the number of characters specified by linebreakCharacterLimit and replacing those with
    returnedText*/
    ...
    public Line[] getLines(int folio) throws SQLException {
        Connection j = null;
        PreparedStatement qry = null;
        try {
    ...
    public String getFolioDropdown() throws SQLException {
        ...
        for (int i = 0; i < allFolios.length; i++) {
            ...
            toret += "<option value=\"" + allFolios[i].getFolioNumber() + "&projectID=" + this.projectID + "\">" + textdisplay.Archive.getShelfMark(allFolios[i].getArchive()) + " " + allFolios[i].getCollectionName() + " " + allFolios[i].getPageName() + "</option>";
        }
        return toret;
    }
...
/**Check for tags used in the transcription that arent part of the schema. This is can be used to warn users during pipeline export that they are including unsupported tags
 without expecting the document to be fully schema compliant.*/
    public String[] hasTagsOutsideSchema() throws SQLException, MalformedURLException, IOException {
        ...
        String[][] res = TagButton.getTagsFromSchema(this, new Vector());
...
/**Associate this project with a project pipeline. This changes the character hotkeys, buttons, and schema and adds a user to this project.*/
    ...
    /**Delete the Project information for this Project. Does not delete the underlying data, as Im sure the next
    request Ill get is to undelete something...returns false if it failed to delete for some reason.*/
...
/**@Depricated use the constructor instead*/
    ...
    /**Get the unique identifier of the next page image to be displayed according to the sequence being used by this Project*/
