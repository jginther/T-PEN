...
/**
 * Methods for retrieving information about a single Archive (IPR for example) along with lots of old, deprecated image url code.
 */
public class Archive {
    ...
    public enum connectionType{httpAuth, cookie, none,local};
...
//                {
//                    this.connectionMethod=connectionType.httpAuth;
//                }
//                cookieURL=rs.getString("cookieURL");
//                if(connectionMethod==connectionType.none && cookieURL!=null && cookieURL.length()>0)
//                {
//                    this.connectionMethod=connectionType.cookie;
//                }
//                if(rs.getBoolean("local"))
//                {
//                    this.connectionMethod=connectionType.none;
//                }
            }
        }
        finally
    ...
    /**
     * Get a status message about the repository. Could be soemthing like 'performance is lousy' or 'server is down'
     * 
     * @return String
     * @throws SQLException 
     */
    ...
    /**
     * Retrieve the IPR agreement text that users must agree to in order to view images from this hosting Archive.
     * 
     * @return String
     * @throws SQLException 
     */
    ...
    /**
     * Set the IPR agreeement text that users must agree to in order to view images from the particular hosting Archive
     * 
     * @param agreement
     * @throws SQLException 
     */
    ...
    @deprecated  use Manuscript.getCities() to get a list of cities, then do getManuscriptsByCity(String city) for each one*/
    ...
    /**@deprecated use Folio object
     * Get the url where an unscaled version of the image can be found*/
    public static String getURL(int folioNum) throws SQLException {
        ...
        try {
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                if (rs.getString("archive").compareTo("ecodices") == 0 || rs.getString("archive").compareTo("CEEC") == 0) {
                    return rs.getString("imageName");
                } else {
                    String toret = (rs.getString("uri").replace(" ", "%20").replace(",", "%2c"));
                    if (!toret.contains("http")) {
                        toret = Folio.getRbTok("SERVERURL") + toret;
                        toret = toret.replace("//", "/").replace("TPEN/", "");
                        toret = toret.replace("http:/", "http://");
                        return toret;
                    }
                    return toret;
                }
     ...
     * @return 
     */
    public static String getURLResized(int folioNum, int size) {
        String toret = "";
        
        String url = "imageResize";
        url += "?folioNum=" + folioNum;
        url += "&height=" + size;
        return url;
    }
     ...
     * @return String
     */
    public static String getURLResizedLarge(int folioNum, int size) {
        String url = "imageResize";
        url += "?folioNum=" + folioNum;
    ...
    /**@deprecated use Manuscript object instead
     * Retrieve the citation that should be printed with images from this Archive*/
    ...
    /**@deprecated use Manuscript object instead
     * Build a proper shelfmark for this image*/
    ...
    /**
     * This function is used to add all manuscripts from an Archive to TPEN. It is not generalized, so modification is required for each new repository.
     * @param archive
     * @param id
     * @param cid
     * @return
