    ...
    <head>
        ...
        <script>
            ...
            $(".imagePreview").on({
                ...
                mouseleave: function(){
                    $("#imagePreview").fadeOut(1000).unbind("load");
    ...
    %> <body>
        ...
        <form action="projSequence.jsp?projectID=<%out.print(""+projID);%>" onsubmit="return checkBeforeSave();" method="POST" style="position:relative;min-height:400px;">
            ...
            <div>
                ...
                <ul id="manuscriptViewer">
                    ...
                    for(int i=0;i<folios.length;i++) {
                        out.print("<li class='folios' title='"+folios[i].getCollectionName()+" "+folios[i].getPageName()+"'><span class=\"clicked ui-icon ui-icon-arrowthick-1-e right\" title='Move a single image into the holding area'></span><span class=\"discard ui-icon ui-icon-trash right\" title='Discard a single folio'></span><span class=\"imagePreview ui-icon left\" data-imgUrl="+folios[i].getImageURL(folios[i].getFolioNumber()) +"></span><input type=\"hidden\" name=\"folio[]\" id=\""+folios[i].getFolioNumber()+"\" value=\""+folios[i].getFolioNumber()+"\"/><span class='collection'>"+folios[i].getCollectionName()+"</span> <span class='pageName'>"+folios[i].getPageName()+"</span></li>\n");
                    }%>
                </ul>
                <div class="small">
            ...
            <input class="ui-corner-all ui-state-default ui-button" type="submit" name="submit" value="Save Changes" id="submit"/>
                ...
                <a class="returnButton" href="project.jsp?projectID=<c:out value="${param.projectID}" />">Return to project page</a><br>
            </div>
        </div>
