...
var lineBtnLast = '<a class="nextLine wRight ui-corner-all ui-button ui-state-error" onclick="return false;" title="next"><span class="ui-icon ui-icon-circle-close right"></span>End of Page</a>';
// user interface
...
//                    isUnsaved = false;
     ...
     */
     splitLine: function(e,event){
        ...
        var newClick = event.pageY - e.offsetTop;               //accomodates browser inconsistencies
...
var Interaction = {
     ...
     */
    ping: function(){
        window.setTimeout(this.heartbeat, 10000);
    },
    /**
     ...
     */
     splitScreen: function(tool,width){
    ...
    zoomBookmark: function(zoomOut){
        var imgSrc = $("#imgTopImg").attr("src");
        if (imgSrc.indexOf("quality") === -1) {
            imgSrc += "&quality=100";
            $("#imgTopImg").attr("src",imgSrc);
        }
        var WRAPWIDTH = $("#wrapper").width();
        ...
        var imgDims = new Array ($("#imgTopImg").height(),$("#imgTopImg").width(),parseInt($("#imgTopImg").css("left")),parseInt($("#imgTopImg").css("top"))-bookmarkTop);
        var imgBottomDims = new Array ($("#imgBottom img").height(),$("#imgBottom img").width(),parseInt($("#imgBottom img").css("left")),parseInt($("#imgBottom img").css("top"))-bookmarkTop);
        if (!zoomOut){
            //zoom in
            $("#bookmark").hide();
            zoomMemory = [parseInt($("#imgTopImg").css("top")),parseInt($("#imgBottom img").css("top"))];
            $("#imgTop").css({
            ...
            $("#imgBottom").css({
                "width"     : imgBottomDims[1] * zoomRatio / WRAPWIDTH * 100 + "%"
            });
            $("#imgBottom img").css({
                "left"      : -bookmarkLeft * zoomRatio,
    ...
    magnify: function(img){
        $("#"+img).bind("mousemove",function(event){
            Interaction.mouseZoom(event, $(this));
        });
    },
    /** 
     ...
     */
    mouseZoom: function(event,img){
        isMagnifying = true;
        var toolDiv = $("#"+liveTool+"Split");
        ...
        //.appendTo(toolDiv)
        .fadeIn().bind("click",function(event){
            Interaction.clickThrough(event, $(this),$img);
        });
    ...
    $(".exitPage").click(function(event){
        ...
        if(isUnsaved()) {
            ...
            $("body").ajaxStart(function(){
                ...
                if(thisLink.indexOf("?") != -1) thisLink += "&tool="+liveTool;
                document.location = thisLink;
            });
            Data.saveTranscription();
            if(leftovers != undefined)Linebreak.saveLeftovers(escape(leftovers));            
        }
    });
    //IPR links
...
//FIXME this does not actualy detect success
    ...
    $(".magnifyBtn").click(function(event){
        if ($(this).hasClass("ui-state-active")){
            $('#zoomDiv').fadeOut();
            isMagnifying = false;
            $("#"+$(this).attr("magnifyImg")).unbind("mousemove");
    ...
    $(document)
        .keydown(function(event){
            ...
            if(event.which == 9 && !inForm){
                ...
                else  focusItem[1].find(".nextLine").click();
            }
            //F1 for help
            if (event.which == 112) {
                event.preventDefault();
...
//        animateEasing:'linear',
    ...
    $("#abbrevBtn").click(function(){
        ...
        isFullscreen = false;
        var toolWidth = $("#abbrevImg").width();
        toolWidth = Screen.limit(toolWidth);
        Interaction.splitScreen("abbrevSplit",toolWidth);
    ...
    $("#imageBtn").click(function(){
        ...
        Interaction.splitScreen ("imageSplit",toolWidth);
    ...
    $("#compareBtn").click(function(){
        ...
        Interaction.splitScreen("compareSplit",toolWidth);
    ...
    $("#historyBtn").click(function(){
        ...
        Interaction.splitScreen("historySplit",toolWidth);
        $("#historyBookmark").empty();
        History.showLine(focusItem[1].attr("data-lineid"));
    });
    ...
    $(".iframeTools").click(function(){
        ...
        Interaction.splitScreen("split"+toolIndex,toolWidth);
    });
    $("#linebreakBtn").click(function(){
        liveTool = "linebreak";
        Interaction.activateToolBtn(this);
        if(isFullscreen)isUnadjusted = true;
        isFullscreen = false;
//        $("#linebreakDiv").width($("#linebreakSplit").width());
        Interaction.splitScreen("linebreakSplit",350);
        return false;
    });
