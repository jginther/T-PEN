 ...
 *
 * SCIAT note: find "SCIAT" to customize settings
 */
        ...
        Editor.init = function() {
            ...
            var updateContextPanel = function() {
                ...
                $('#selected_panel, #multiselected_panel, #g_panel, #rect_panel, #circle_panel,\
					#ellipse_panel, #line_panel, #text_panel, #image_panel, #anno_data_panel').hide();
                if (elem != null) {
                    if (!elem.parentElement.id) { // only if a full element is chosen
                        var annotationText = elem.getAttribute('text_annotation');
                        if (annotationText == 'undefined') annotationText = null;
                        $('#annotationText').val(annotationText);
                        $('#anno_data_panel').show();
                    }
			...
			*/
            ...
            var setIconSize = Editor.setIconSize = function(size, force) {
                ...
                var cssResizeRules = {
                    ...
                    "input.spin-button": {
                        'background-image': {
                            l: "url('images/spinbtn_updn_big.png')", 
                            xl: "url('images/spinbtn_updn_big.png')"
                        },
                        'background-position': {
                            l: '100% -5px', 
                            xl: '100% -2px'
            ...
            setTimeout(function() {
                //SCIAT update path
                svgCanvas.embedImage('js/sciat/images/interface/logo.png', function(datauri) {
                    if(!datauri) {
                        // Disable option
            ...
            var Actions = function() {
...

                {
                    ...
                    {
                    sel:'#tool_source_cancel,#svg_source_overlay,#tool_docprops_cancel', 
                    fn: cancelOverlays, 
                    evt: 'click', 
                    key: ['esc', false, false], 
                    hidekey: true
...
return {
    setAll: function() {
        ...
        $('.attr_changer, #image_url').bind('keyup', 'return', 
            function(evt) {
                $(this).change();
                evt.preventDefault();
            }
