...
/*                        -moz-column-width:400px;-moz-column-gap:20px;-webkit-column-width:400px;-webkit-column-gap:20px;column-width:400px;column-gap:20px;*/
            ...
            #helpBtns,#closeHelp {position: absolute;z-index: 1;right:5px;top:5px;}
            #closeHelp:hover {background-image: url(css/custom-theme/images/ui-icons_cd0a0a_256x240.png);}
            #helpMe{cursor: help;margin: -4px;}
...
/*            .showAnno {background-color: rgba(255,255,0,.4)}*/
            ...
            .btnTitle {position: absolute;z-index: 6;top:-22px;left:0;white-space: nowrap;}
    ...
    <body id="transcriptionPage">
        ...
        <div id="wrapper" class="ui-corner-all">
            ...
            <div id="imgTop">
                ...
                <div id="workspace">
                    <div id="helpBtns">
                    <a id="helpMe" class="wBtn listBegin ui-state-default ui-button">Help<span class="ui-icon ui-icon-help right"></span></a>
                    <a id="helpContact" href="admin.jsp#aboutTab" target="_blank" class="wBtn listEnd ui-button ui-state-default ">Contact<span class="ui-icon ui-icon-comment right"></span></a>
                    </div>
                <div id="captions">
...
<%}%>
                    ...
                    for (int i=0;i<projectTools.length;i++){%>
                        ...
                        <div id="navOptions" class="right">
                            <%if (thisProject.getPreceedingPage(pageno) > 0) {%>
                            <a id="prevPage" title="Previous Page" class="exitPage wBtn" href="?p=<%out.print(thisProject.getPreceedingPage(pageno) + "&projectID=" + projectID);%>">Previous&nbsp;Page<span class="ui-icon ui-icon-seek-prev left"></span></a>
                            <%}
                            if (thisProject.getFollowingPage(pageno) > 0) {%>
                            <a id="nextPage" title="Next Page" class="exitPage wBtn" href="?p=<%out.print(thisProject.getFollowingPage(pageno) + "&projectID=" + projectID);%>">Next&nbsp;Page<span class="ui-icon ui-icon-seek-next right"></span></a>
                            <%}%>
                        </div>
                        <div id="siteNavigation" class="left right">
...
<%}%>
                            <a class="exitPage wBtn" title="T-PEN Home" href="index.jsp"><span class="left ui-icon ui-icon-home"></span>Home</a>
                        </div>
                        <div id="projectOptions" class="left right">
                            ...
                            <%}%>
<!--                            <a id="saveBtn" title="Save all lines on this page" name="saveBtn"><span class="left ui-icon ui-icon-disk"></span>Save</a>-->
                        </div>
                    ...
                    <div id="historyListing">
                       ...
                       //             m.set(Calendar.MILLISECOND, 0);
                        ...
                        for(int i=0;i<thisText.length;i++){
                            textdisplay.ArchivedTranscription[] history = ArchivedTranscription.getAllVersions(thisText[i].getLineID());
                            if (history.length == 0){
                                out.print("<div class='historyLine' id='history"+thisText[i].getLineID()+"' linewidth='"+thisText[i].getWidth()+"' lineheight='"+thisText[i].getHeight()+"' lineleft='"+thisText[i].getX()+"' linetop='"+thisText[i].getY()+"'>No previous versions</div>");
                            } else {
                                out.print("<div class='historyLine' id='history"+thisText[i].getLineID()+"'>");
                                for (int j=0;j<history.length;j++){%>
                                <div class="historyEntry ui-corner-all" linewidth='<%out.print(history[j].getWidth());%>' lineheight='<%out.print(history[j].getHeight());%>' lineleft='<%out.print(history[j].getX());%>' linetop='<%out.print(history[j].getY());%>'>
                                    <%
                                    String dateString = "-";
                                    DateFormat dfm;
                                    Calendar historyDate = Calendar.getInstance();
                                    historyDate.setTimeInMillis(history[j].getDate().getTime());
                                    if ((YEAR == historyDate.get(Calendar.YEAR)) && (DOY == historyDate.get(Calendar.DAY_OF_YEAR))){ // not perfect, but other date comparisons were frustrating
                                        ...
                                        dateString = dfm.format(history[j].getDate());//DateFormat.getDateInstance(DateFormat.MEDIUM).format(historyDate);
                                    }
                                    dfm.setCalendar(historyDate);
                                    %>
                                    <div class="historyDate"><%out.print(dateString);%></div>
                                    <% if (history[j].getCreator() > 0){
                                        User creatorUser = new user.User(history[j].getCreator());
                                    ...
                                    <div class="historyText"><%out.print(history[j].getText());%></div>
                                    <div class="historyNote"><%out.print(history[j].getComment());%></div>
<%if(isMember || permitModify){%>
