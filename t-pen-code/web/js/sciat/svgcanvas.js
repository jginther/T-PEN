...
$.SvgCanvas = function(container, config)
{
    ...
    var ChangeElementCommand = this.undoCmd.changeElement = function(elem, attrs, text) {
        ...
        this.apply = function() {
            ...
            for(var attr in this.newValues ) {
                ...
                else if (attr == "stdDeviation") {
                    canvas.setBlurOffsets(this.elem.parentNode, this.newValues[attr]);
                }
        ...
        // SCIAT adding element to save
        if(!this.elem.getAttribute('text_annotation')) 
            this.elem.setAttribute('text_annotation');
    ...
    (function(c) {
        ...
        c.undoMgr = {
            ...
            getRedoStackSize: function() {
                return undoStack.length - undoStackPointer;
            },
    ...
    (function() {
        ...
        function Selector(id, elem) {
            ...
            this.selectorGroup = addSvgElementFromJson({
                "element": "g",
                "attr": {
            ...
            this.resize = function() {
                ...
                assignAttributes(this.rotateGripConnector, {
                    x1: nbax + (nbaw)/2, 
                    y1: nbay, 
                    x2: nbax + (nbaw)/2, 
                    y2: nbay- 20
                    });
                assignAttributes(this.rotateGrip, {
                    cx: nbax + (nbaw)/2, 
                    cy: nbay - 20
                });
    ...
    function getElem(id) {
        ...
        if(svgroot.querySelector) {
            ...
            // xpath lookup
            return svgdoc.evaluate('svg:svg[@id="svgroot"]//svg:*[@id="'+id+'"]', container, function() {
...
(function() {
        ...
        switch (current_mode)
        {
            case "select":
                ...
                if (selectedElements[0] != null) {
                    ...
                    if(evt.shiftKey) {
                        var xya = Utils.snapToAngle(start_x,start_y,x,y);
                        x=xya.x;
                        y=xya.y;
                    }
            ...
            case "line":
                ...
                if(evt.shiftKey) {
                    var xya=Utils.snapToAngle(start_x,start_y,x2,y2);
                    x2=xya.x;
                    y2=xya.y;
                }
    ...
    var pathActions = this.pathActions = function() {
        ...
        function Path(elem) {
            ...
            this.endChanges = function(text) {
                ...
                var cmd = new ChangeElementCommand(elem, {
                    d: p.last_d
                    }, text);
                ...
                if(current_mode == "path") {
                    ...
                    if (current_path_pts.length == 0) {
                        ...
                        // else, create a new point, append to pts array, update path element
                            ...
                            if(evt.shiftKey) {
                                var xya=Utils.snapToAngle(lastx,lasty,x,y);
                                x=xya.x;
                                y=xya.y;
                            }
    ...
    this.setSvgString = function(xmlString) {
        try {
            ...
            // are removed
            walkTreePost(svgcontent, function(n){
                try{
...
var identifyLayers = function() {
    ...
    if (orphans.length > 0) {
        var i = 1;
        while ($.inArray(("Layer " + i), layernames) != -1) {
            i++;
        }
        var newname = "Layer " + i;
    ...
    walkTree(current_layer, function(e){
        e.setAttribute("style","pointer-events:inherit");
    });
...
var changeSelectedAttributeNoUndo = function(attr, newValue, elems) {
    var handle = svgroot.suspendRedraw(1000);
    if(current_mode == 'pathedit' && attr != "text_annotation") {
        // Editing node
